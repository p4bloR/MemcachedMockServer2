...
#=end
  context "when given existing keys" do
    it "a single one returns 'VALUE' + flag + bytes + value" do
      key = "word"
      flag = 0
      ttl = 9
      bytes = 5
      ...
      expect(@client.read).to be == "#{SystemStrings.value} #{key} #{flag} #{bytes}"
    ...
    it "2 keys are returned sequentially" do
      ...
      expect(@client.read).to be == "#{SystemStrings.value} #{key2} #{flag2} #{bytes2}"
    ...
    it "2 keys are returned sequentially" do
      ...
      key1, flag1, ttl1, bytes1, value1 = 'word', 0, 9, 5, 'hello'
      key2, flag2, ttl2, bytes2, value2 = 'z', 5, 3, 3, 'bye'
    ...
    it "3 keys are returned sequentially" do
      ...
      key1, flag1, ttl1, bytes1, value1 = 'r', 2, 7, 10, '0123456789'
      key2, flag2, ttl2, bytes2, value2 = 't', 4, 2, 1, 'K'
      key3, flag3, ttl3, bytes3, value3 = 'y', 9, 5, 4, '[%<]'
      ...
      expect(@client.read).to be == "#{SystemStrings.value} #{key2} #{flag2} #{bytes2}"
      expect(@client.read).to be == value2   
      expect(@client.read).to be == "#{SystemStrings.value} #{key3} #{flag3} #{bytes3}"
