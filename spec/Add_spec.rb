...
RSpec.describe "Add command" do
  ...
  context "given a correct format" do
    it "stores previously non existing keys" do
    ...
    it "doesn't overwrites previously existing keys" do
      key = 'a_key'
      ...
      expect(@client.read).to be == "#{SystemStrings.value} #{key} #{flag1} #{bytes1}"
      expect(@client.read).to be == value1         
      expect(@client.read).to be == SystemStrings.end      
    end  
  end
